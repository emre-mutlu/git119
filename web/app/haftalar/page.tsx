          ...
          {weekSlugs.map((slug) => {
            ...
            return (
              <Link key={slug} href={href} className="group block">
                <div className="relative bg-slate-900/60 backdrop-blur-sm border border-slate-800 hover:border-violet-500/50 p-5 rounded-xl transition-all duration-300 overflow-hidden
                  ...
                  <div className="relative flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-violet-600/20 to-indigo-600/20 rounded-lg flex items-center justify-center border border-violet-500/20 group-hover:border-violet-500/40 transition-colors">
                        <span className="text-xl font-bold text-violet-400">{parseInt(weekNum)}</span>
                      </div>
                      <div>
                    ...
                    <ArrowRight size={20} className="text-slate-600 group-hover:text-violet-400 group-hover:translate-x-1 transition-all flex-shrink-0" />
                  </div>
                </div>
              </Link>
            );
